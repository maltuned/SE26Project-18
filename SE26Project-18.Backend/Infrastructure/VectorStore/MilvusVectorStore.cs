...
internal sealed class MilvusVectorStore : IVectorStore, IDisposable
{
    ...
    private readonly MilvusClient? _databaseAdminClient;
    ...
    public MilvusVectorStore(IOptions<MilvusOptions> options, ILogger<MilvusVectorStore> logger)
    {
        ...
        if (!string.Equals(configuration.DatabaseName, "default", StringComparison.Ordinal))
        {
            _databaseAdminClient = string.IsNullOrWhiteSpace(configuration.UserName)
                ? new MilvusClient(
                    configuration.HostName,
                    configuration.Port,
                    configuration.UseTls,
    ...
    )
    {
        ...
        _logger.LogInformation(
            "Milvus upsert started: collection {CollectionName}, records {RecordCount}",
            definition.Name,
            recordList.Count
        );
        try
        {
            ...
            _logger.LogInformation(
                "Milvus upsert completed: collection {CollectionName}, records {RecordCount}, elapsed {ElapsedMilliseconds} ms",
                definition.Name,
                recordList.Count,
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            _logger.LogError(
                exception,
                "Milvus upsert failed: collection {CollectionName}, records {RecordCount}, elapsed {ElapsedMilliseconds} ms",
                definition.Name,
                recordList.Count,
                stopwatch.ElapsedMilliseconds
            );
    ...
    )
    {
        ...
        var stopwatch = Stopwatch.StartNew();
        var allowedIdCount = request.AllowedIds?.Count ?? 0;
        _logger.LogInformation(
            "Milvus search started: collection {CollectionName}, field {VectorField}, limit {Limit}, allowed IDs {AllowedIdCount}",
            request.IndexName,
        ...
        try
        {
            ...
            var mapped = MapSearchResults(request.IndexName, result);
            _logger.LogInformation(
                "Milvus search completed: collection {CollectionName}, results {ResultCount}, elapsed {ElapsedMilliseconds} ms",
                request.IndexName,
                mapped.Count,
                stopwatch.ElapsedMilliseconds
            );
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            _logger.LogError(
                exception,
                "Milvus search failed: collection {CollectionName}, limit {Limit}, allowed IDs {AllowedIdCount}, elapsed {ElapsedMilliseconds} ms",
                request.IndexName,
                request.Limit,
                allowedIdCount,
                stopwatch.ElapsedMilliseconds
    ...
    )
    {
        ...
        var stopwatch = Stopwatch.StartNew();
        _logger.LogInformation(
            "Milvus delete started: collection {CollectionName}, IDs {IdCount}",
            indexName,
            ids.Count
        );
        try
        {
            ...
            _logger.LogInformation(
                "Milvus delete completed: collection {CollectionName}, IDs {IdCount}, elapsed {ElapsedMilliseconds} ms",
                indexName,
                ids.Count,
                stopwatch.ElapsedMilliseconds
            );
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            _logger.LogError(
                exception,
                "Milvus delete failed: collection {CollectionName}, IDs {IdCount}, elapsed {ElapsedMilliseconds} ms",
                indexName,
                ids.Count,
                stopwatch.ElapsedMilliseconds
            );
    ...
    {
        if (_databaseAdminClient is null)
        {
            return;
        }
        ...
        try
        {
            var databases = await _databaseAdminClient.ListDatabasesAsync(ct);
            if (databases.Contains(_databaseName, StringComparer.Ordinal))
            {
                return;
            }
            ...
            try
            {
                await _databaseAdminClient.CreateDatabaseAsync(_databaseName, ct);
                _logger.LogInformation("Created Milvus database {DatabaseName}", _databaseName);
            }
            catch (MilvusException)
            {
                databases = await _databaseAdminClient.ListDatabasesAsync(ct);
                if (!databases.Contains(_databaseName, StringComparer.Ordinal))
                {
                    throw;
                }
