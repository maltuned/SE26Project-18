...
export default function RecruitmentManageScreen() {
  ...
  const initialRecruitment = useMemo(() => {
    try {
      if (params.data) {
  ...
  const [recruitment, setRecruitment] = useState<RecruitmentData | null>(
    initialRecruitment || null,
  );
  const [closed, setClosed] = useState(initialRecruitment?.status === "已关闭");
  ...
  useEffect(() => {
    const recruitmentId = initialRecruitment?.id;
    if (recruitmentId) {
      getRecruitmentById(recruitmentId).then((data) => {
        if (data) {
          setRecruitment(data);
      ...
      getResponses(recruitmentId).then((data) => {
        setResponses(data);
        setLoadingResponses(false);
      });
    }
  }, [initialRecruitment?.id]);
  ...
  const handleViewChat = async (res: ResponseData) => {
    ...
    if (chat) {
      ...
      router.push(`/recruitment-manage?id=${recruitment.id}`);
    ...
    <View style={[styles.container, { backgroundColor: colors.surface }]}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            router.push(
              `/recruitment-edit?data=${encodeURIComponent(JSON.stringify(recruitment))}`,
            )
          }
        >
